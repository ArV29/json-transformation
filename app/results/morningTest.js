...
res.CustomerCity = source.address.city;
...
res.CommercialLoans= [];
source.loanHistory.forEach(item => {(item.isCommercial == true)?res.CommercialLoans.push(item):res.CommercialLoans.push()});
console.log(res);
var fs = require('fs');
jsonData = JSON.stringify(res);
fs.writeFile("./results/morningTest.json", jsonData, function (err) {
	if(err){
		console.log(err);
	}
});